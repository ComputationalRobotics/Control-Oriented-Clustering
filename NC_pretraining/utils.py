...
class RealPushTImageDataset(torch.utils.data.Dataset):
    def __init__(self,
                 dataset_path: str,
                 pred_horizon: int,
                 obs_horizon: int,
        ...
        # float32, [0,255], (N,224,224,3)
        # DO NOT /255 here because PIL Image class needs raw RGB values

        train_image_data = dataset_root['data']['camera_0'][:num_max_frames]
        """
        with open('.npy', 'rb') as f: 
            label_lists = list(np.load(f))
        """
        ...
        #crop_w_1 = 35
        #crop_w_2 = 259
        #train_image_data = train_image_data[:, crop_h_1:crop_h_2, crop_w_1:crop_w_2, :]
        
        # (N, D)
        train_data = {
            # first two dims of state vector are agent (i.e. gripper) locations
            'agent_pos': dataset_root['data']['robot_eef_pose'][:num_max_frames, 0, :2],
            'action': dataset_root['data']['action'][:num_max_frames, 0, :2]
        }
        episode_ends = dataset_root['meta']['episode_ends'][:num_max_demos]
        ...
        # images are already normalized
        normalized_train_data['image'] = train_image_data
        #normalized_train_data['action_intention'] = np.array(label_lists)
    ...
    def __getitem__(self, idx):
        ...
        nsample['agent_pos'] = nsample['agent_pos'][:self.obs_horizon,:]
        nsample["id"] = self.id
        #nsample['action_intention'] = action_intention
...
class RealPushTImageDatasetCaliberated(torch.utils.data.Dataset):
    def __init__(self,
                 dataset_path: str,
                 pred_horizon: int,
                 obs_horizon: int,
        ...
        with open("/home/turing/Han/feature_analysis/real_world_NC_regularization_training_100demos_fixed_range/adap_diffusion_policy/real_world_action_labels_100demos_8classes_input_space_theta_fixed.npy", 'rb') as f: 
            label_lists = list(np.load(f))
